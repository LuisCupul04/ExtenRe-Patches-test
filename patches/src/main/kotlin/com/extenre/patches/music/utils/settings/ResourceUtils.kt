...
import com.extenre.patches.music.utils.patch.PatchList
import com.extenre.util.adoptChild
import com.extenre.util.cloneNodes
...
internal object ResourceUtils {
                ...
                .forEach {
                    if (!isIncludedCategory(category)) {
                        it.adoptChild(PREFERENCE_SCREEN_TAG_NAME) {
                            setAttribute(
                                "android:title",
                                "@string/extenre_preference_screen_$category" + "_title"
                            )
                            setAttribute("android:key", "extenre_preference_screen_$category")
                        }
        ...
        context.document(SETTINGS_HEADER_PATH).use { document ->
            document.doRecursively node@{
                ...
                it.getAttributeNode("android:key")?.let { attribute ->
                    if (attribute.textContent == "extenre_preference_screen_$category") {
    ...
    fun addGmsCorePreference(
        ...
        context.document(SETTINGS_HEADER_PATH).use { document ->
            ...
            List(tags.length) { tags.item(it) as Element }
                .filter {
                    it.getAttribute("android:key").contains("extenre_preference_screen_$category")
                }
                .forEach {
